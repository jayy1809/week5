...
For each branch, evaluate the environmental impact and then synthesize the insights into a final conclusion."""
    ...
    results = await asyncio.gather(
        make_groq_request(zero_shot_messages),
        make_groq_request(few_shot_messages),
        make_groq_request(cot_messages),
        make_groq_request(meta_messages),
        make_groq_request(tot_messages)
    )
...
Please synthesize these findings into a final comparison."""
    ...
    return {
        "zero_shot": results[0],
        "few_shot": results[1],
        "chain_of_thought": results[2],
        "meta": results[3],
        "tree_of_thoughts": results[4],
        "prompt_chaining": final_chain_response
    }
...
if __name__ == "__main__":
    ...
    print(f"Total time taken: {time.time() - s} seconds")